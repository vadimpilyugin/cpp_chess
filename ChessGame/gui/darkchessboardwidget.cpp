...
QPixmap getPixmapForPiece(PieceType type,ChessColor color){
...
void DarkChessBoardWidget::mousePressEvent(QMouseEvent *event){
    if (event->button() == Qt::LeftButton) {
        ...
        if(piece.type!=PieceType::None && !child->isHidden()){
            ...
            QString str=(toString(piece.type)+","+toString(piece.color)+",").c_str()+QString::number(xcoord)+","+QString::number(ycoord);
...
void DarkChessBoardWidget::dropEvent(QDropEvent *event)
{
    if (event->source() == this) {
        ...
        if(row>=1 && col>=1 && row<=1+rowCount && col<=1+colCount){
            ...
            Piece piece;
            piece.type=toPiece(strData[0].toStdString());
